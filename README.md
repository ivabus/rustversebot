...
- `BOT_CONFIG_PATH` — путь к конфигурации событий (по умолчанию
  `config.toml`);
...
- `BOT_ANNOUNCEMENT_POLL_SECS` — период проверки production-индекса Nanoka
  (по умолчанию `3600`, минимум `60`).

Данные будущих сезонов, характеристики противников и ссылки на изображения бот
получает через crate `nanoka`. Каждая карточка рендерится один раз на сезон и
отправляется каждому чату с зарегистрированными UID не более одного раза.
...
`TURSO_DATABASE_URL`. Nix wrapper автоматически использует упакованный
`config.toml`, если `BOT_CONFIG_PATH` не задан явно.
