//! Shared card view models and data preparation.
...
pub(crate) fn wrap_game_text_lines(text: &str, max_width: usize) -> Vec<String> {
...
mod tests {
    ...
    use crate::reference_svg::{MJ_ENVIRONMENT, rasterize_svg, shiyu_info};
    ...
    fn compatibility_renderer_scales_logical_dimensions_per_render() {
        ...
        for (zoom_factor, expected_dimensions) in [
            ...
            let png = rasterize_svg(svg, RenderScale::new(zoom_factor).unwrap()).unwrap();
    ...
    fn shiyu_info_fixture_renders_as_png() {
        let png = shiyu_info(&shiyu_fixture()).unwrap();
    ...
    fn renderer_rejects_remote_images_that_were_not_preloaded() {
        ...
        let error = crate::reference_svg::try_render_from_serialize("da.j2", &data).unwrap_err();
